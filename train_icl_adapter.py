...
class FlamingoICLAdapter(pl.LightningModule):
    def __init__(self, interface, lr, weight_decay=1e-2, warm_steps=0.1):
        ...
        for p in self.interface.model.parameters():
            p.requires_grad = False
        self._init_adapter(50280)
        self.loss_fct = torch.nn.CrossEntropyLoss(
            reduction='none', ignore_index=self.interface.pad_token_id
        )

    def _init_adapter(self, n):
        self.adapter = nn.Sequential(nn.Linear(n, 128), nn.LeakyReLU(), nn.Linear(128, n)).to('cuda')
    ...
    def forward(self, model_input, ice_token_length):
        ...
        for i in range(bs):
            ...
            adapter_logits = self.adapter(neg_logits) + neg_logits
            best_logits = best_logits.repeat(aug_num - 1, 1, 1)
            best_adapter_logits = self.adapter(best_logits) + best_logits
            neg_num, cur_query_length, vocab_size = adapter_logits.shape

            neg_remain_loss = calculate_kl_divergence(
                adapter_logits.reshape(neg_num * cur_query_length, -1),
                neg_logits.reshape(neg_num * cur_query_length, -1),
            ) 
            
            best_remain_loss = calculate_kl_divergence(
                best_adapter_logits[0].reshape(cur_query_length, -1),
                best_logits[0].reshape(cur_query_length, -1),
            )
            remain_loss = neg_remain_loss + best_remain_loss

            soft_loss = calculate_kl_divergence(
                adapter_logits.reshape(neg_num * cur_query_length, -1),
                best_logits.reshape(neg_num * cur_query_length, -1),
            )
            neg_hard_loss = self.loss_fct(
                adapter_logits.view(-1, shift_logits.size(-1)), query_label_idx.view(-1)
            ).mean()
            best_hard_loss = self.loss_fct(
                best_adapter_logits[0].view(-1, shift_logits.size(-1)), query_label_idx[0].view(-1)
            ).mean()
            hard_loss = neg_hard_loss + best_hard_loss
            
            loss_dict['best_hard_loss']+= best_hard_loss
            loss_dict['neg_hard_loss'] += neg_hard_loss
            loss_dict['best_remain_loss'] += best_remain_loss
            loss_dict['neg_remain_loss'] += neg_remain_loss
            loss_dict['hard_loss'] += hard_loss
            loss_dict['remain_loss'] += remain_loss
            loss_dict['soft_loss']+= soft_loss
            loss_dict['loss'] += (soft_loss + hard_loss + remain_loss) / 3
