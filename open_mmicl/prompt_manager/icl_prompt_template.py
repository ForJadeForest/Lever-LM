...
from .utils import _check_type_list, _check_dict
...
class PromptTemplate:
    ...
    def __init__(
        ...
        if (
            (self.selected_column_name is not None and self.selected_column_map is None)
            or self.selected_column_name is None
            and self.selected_column_map is not None
        ):
            raise ValueError(
                "self.selected_column_name and self.selected_column_map should be set together"
            )
    ...
    def _check_template_legacy(self):
        if isinstance(self.template, Dict):
            ...
            for tp_dict_val in self.template.values():
                if not isinstance(tp_dict_val, str):
                    raise TypeError(
                        f"dictionary of template expects a str value, but got '{tp_dict_val}'"
                    )
        ...
        # Check duplicates
        if len(self.column_token_map.values()) != len(
            set(self.column_token_map.values())
        ):
            raise ValueError(f"There are duplicates in self.column_token_map.values()")
        if (
            self.ice_token is not None
            and self.ice_token in self.column_token_map.values()
        ):
            raise ValueError(
                f"There are duplicates between self.column_token_map.values() and self.ice_token"
            )
    ...
    def generate_ice_item(self, entry: Dict, label: Hashable) -> str:
        ...
        for key, token in self.column_token_map.items():
            if (
                self.selected_column_map is not None
                and key == self.selected_column_name
            ):
    ...
    def generate_label_prompt_item(
        self, entry: Dict, ice: str, label: Hashable, remain_sep: Optional[bool] = False
    ) -> str:
        ...
        if self.ice_token is None:
            raise ValueError(
                "PromptTemplate.ice_token should be not None when generates prompt"
            )
        # Select the corresponding template
        tp = self.template[label] if isinstance(self.template, Dict) else self.template
        # Remove sep token
