...
run_train() {
    ...
    if [ "${task}" == "vqa" ]; then
        echo "==========Begin: ${ex_name_prefix}_${ex_name_suffix}-ICD-LM: ${icd_lm}==========" 
        python train.py train="${icd_lm}" \
            data_files="${data_file}" \
            epochs=20 \
            val_step=${val_step} \
            ex_name="${ex_name_prefix}_${ex_name_suffix}_${icd_lm}" \
        ...
        python train.py train="${icd_lm}" \
            ...
            epochs=20 \
            val_step=${val_step} \
            ex_name="${ex_name_prefix}_${ex_name_suffix}_freeze_adapter_non_norm_${icd_lm}" \
            device_num=${device_num} \
            dataset=${dataset} \
            task=${task} \
...
run_train "${task}-${dataset}-OpenFlamingo-9B-vitl-mpt7b-RandSampler-beam_size:5-few_shot:2-candidate_num:64-sample_num:5000.json" 80
