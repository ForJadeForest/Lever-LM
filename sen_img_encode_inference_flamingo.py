...
from transformers import AutoProcessor, AutoTokenizer, BertTokenizer, GPT2Config

from datasets import load_dataset
from src.datasets import CocoDataset, SenImgEncodeCaptionICLMDataset
from src.models import SenImgEncodeCaptionICLM
...
if __name__ == '__main__':
    ...
    if test_iclm:
        ...
        iclm_model = SenImgEncodeCaptionICLM(lm_config)
        iclm_model.load_state_dict(
            torch.load(
                'result/model_cpk/sen_img_encode_iclm/last-val_loss:11.5342.pth'
            )['model']
        )
        ...
        with torch.inference_mode():
            ...
            for d in tqdm(ds['validation']):
                next_ice_idx_seq = []
                next_ice_input = []
                x_input = clip_img_processor(images=d['image'], return_tensors='pt').to(
                    device
                )['pixel_values']
                for i in range(4):
                    if ice_input:
